...
fn cosine_fixed_point_is_detected() {
    let recurrence = fractal_music::cosine_fixed_point(1.0);
...
#[test]
fn default_config_uses_polar_system() {
    // El caso base debe usar el sistema de coordenadas polar, igual que Python.
    let config = default_concept_config();
    assert_eq!(config.discretization.coordinate_system, "polar");
    assert!(config.discretization.ranges.contains_key("r"));
    assert!(config.discretization.ranges.contains_key("theta"));
    // El resultado debe proyectarse en ejes polares (r, theta).
    let result = run_exploration(&config).unwrap();
    let axes = result.transformer.coordinate_system.axes();
    assert!(axes.contains(&"r"));
    assert!(axes.contains(&"theta"));
}
...
fn session_payload_roundtrip() {
    let config = default_concept_config();
    let case = fractal_music::ExplorationCase {
    ...
    let result = run_exploration(&config).unwrap();
    let data = fractal_music::build_session_data(
        &config,
    ...
    let restored = fractal_music::parse_session_data(&data).unwrap();
